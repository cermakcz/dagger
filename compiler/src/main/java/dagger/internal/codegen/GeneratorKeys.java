...
final class GeneratorKeys {
   ...
   * runtime.
   */
  static String getRuntimeGeneratedMembersKey(TypeMirror type) {
    return "\"members/\" + " + CodeGen.typeToRuntimeGeneratedKeyString(type);
  }

  /**
   * Returns the code for generating the provider key for {@code type} in runtime.
   */
  public static String getRuntimeGeneratedProviderKey(TypeMirror type) {
    return CodeGen.typeToRuntimeGeneratedKeyString(type);
  }

  /**
   * Returns the code for generating the provider key for {@code variable} in runtime.
   */
  static String getRuntimeGeneratedProviderKey(VariableElement variable) {
    StringBuilder result = new StringBuilder();
    AnnotationMirror qualifier = GeneratorKeys.getQualifier(variable.getAnnotationMirrors(),
        variable);
    if (qualifier != null) {
      qualifierToStringRuntimeGenerated(qualifier, result);
      result.append(" + ");
    }
    CodeGen.typeToRuntimeGeneratedKeyString(variable.asType(), result, '.');
    return result.toString();
  }
   ...
   */
  static String getRuntimeGeneratedProviderKey(ExecutableElement method) {
    StringBuilder result = new StringBuilder();
    AnnotationMirror qualifier = getQualifier(method.getAnnotationMirrors(), method);
    if (qualifier != null) {
      qualifierToStringRuntimeGenerated(qualifier, result);
    ...
    CodeGen.typeToRuntimeGeneratedKeyString(method.getReturnType(), result, '.');
    return result.toString();
  }
   ...
   * runtime.
   */
  static String getRuntimeGeneratedElementKey(ExecutableElement method) {
    StringBuilder result = new StringBuilder();
    AnnotationMirror qualifier = getQualifier(method.getAnnotationMirrors(), method);
    if (qualifier != null) {
      qualifierToStringRuntimeGenerated(qualifier, result);
   ...
   */
  private static void qualifierToStringRuntimeGenerated(AnnotationMirror qualifier,
      StringBuilder result) {
    // TODO: guarantee that element values are sorted by name (if there are multiple)
    result.append("\"@\" + ");
    CodeGen.typeToRuntimeGeneratedKeyString(qualifier.getAnnotationType(), result, '.');
    result.append("+ \"(");
